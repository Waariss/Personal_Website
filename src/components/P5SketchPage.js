...
const createSketch = (p) => {
  ...
  function drawbg() {
    const x = p.map(p.mouseX, 0, 100, 200, 200);
    const r = p.map(p.mouseX, 0, 100, 200, 200);
...
const P5SketchPage = () => {
  ...
  useEffect(() => {
    ...
    const instance = new p5(createSketch, containerRef.current);
  ...
  return (
    <>
      <Navigation />
      <div id="p5-container" ref={containerRef} className="p5-sketch-shell" />
      <Footer />
    </>
  );
};

export default P5SketchPage;
